...
## Step 2: 수정 위치 입력 — 항상 `AskUserQuestion` 카테고리 메뉴 (필수)

진입 경로 (직접 호출 / 디테일링 진입 / 카테고리 요청) 무관하게 **항상 `AskUserQuestion` 으로 4 큰 카테고리 메뉴를 띄운다**. 채팅에 "어디를 어떻게 바꿀까요?" 텍스트로만 묻는 것 X — 사용자가 다시 자연어로 답해야 하니 번거로움.

`AskUserQuestion` 의 "Other" 옵션은 자동으로 추가됨 → 사용자가 변수를 이미 알고 있어서 자유 입력하고 싶으면 "Other" 선택 후 자연어 입력. 즉 카테고리 메뉴 + 자유 입력 *모두* 한 화면에서 처리.

### 2-1. 큰 카테고리 메뉴 — `AskUserQuestion` (필수)

수정 대상 파일 경로 한 줄 출력 후 바로:

```
question: "어떤 영역을 만져볼까요? (변수 안다면 Other 자유 입력)"
header: "수정 영역"
multiSelect: false
options:
  - label: "🎨 글자·색"
...
**사용자가 "Other" 선택 시** → 자유 입력 받은 자연어를 Step 3 (변수 식별) 로 바로 전달. 카테고리 sub 메뉴 건너뜀.

**사용자가 4 카테고리 중 하나 선택 시** → 2-2 로 진행.

### 2-2. 카테고리 안 sub 항목 — `AskUserQuestion` (필수)

사용자 선택에 따라 다음 sub-메뉴를 보여준다. 각 sub-카테고리의 변수 표는 `variables.md` 의 해당 섹션 Read 후 첨부.

**모든 sub 메뉴의 `question` 문에 "다른 영역으로 돌아가려면 Other → '뒤로'" 안내 포함**. 사용자가 Other 선택 후 "뒤로" / "다른 카테고리" / "큰 메뉴" 같은 답 입력 시 → 2-1 큰 카테고리 메뉴 다시 호출 (다른 카테고리 선택 가능). Other + 변수명/자연어 답 시 → Step 3 (변수 식별) 진행.

**🎨 글자·색 선택 시**:
```
question: "글자·색 중 어디? (다른 영역 돌아가려면 Other → '뒤로')"
options:
...
**📝 블록 요소 선택 시**:
```
question: "블록 요소 중 어디? (다른 영역 돌아가려면 Other → '뒤로')"
options:
...
**🖼 UI 크롬 선택 시**:
```
question: "UI 크롬 중 어디? (다른 영역 돌아가려면 Other → '뒤로')"
options:
  - "탭·리본"      (--tab-* · --ribbon-*)
...
**🔧 레이아웃·기타 선택 시**:
```
question: "레이아웃·기타 중 어디? (다른 영역 돌아가려면 Other → '뒤로')"
options:
...
(다른 sub 보고 싶으면 "뒤로", 큰 카테고리 처음부터면 "큰 메뉴")
```

사용자 답변 분기:
- **"뒤로"** / "다른 sub" → 2-2 의 같은 큰 카테고리 sub 메뉴 다시 호출
- **"큰 메뉴"** / "다른 영역" / "처음부터" → 2-1 큰 카테고리 메뉴 다시 호출
- **변경 자연어** ("h1 색을 ...") → Step 3 진행
